...
export class DocsCalloutComponent {
  ...
  set href(value: string) {
    const version = packageJson?.config?.gauss_manager_library_short_version;
    const docsUrl = packageJson?.config?.gauss_manager_library_docs_url ?? 'https://github.com/jeffreysSharp';
